...
class A2MLP(nn.Module):
    ...
    def __init__(self, config, x):
        ...
        self.V = nn.Linear(in_features=config.intermediate_size, out_features=config.hidden_size, bias=False)
        self.W = nn.Linear(in_features=config.intermediate_size, out_features=config.hidden_size, bias=False)
        self.W2 = nn.Linear(in_features=config.hidden_size, out_features=config.intermediate_size, bias=False)
        self.Swish_1 = nn.SiLU()

    def forward(self, hidden_states):
        xW = self.W(hidden_states)
        xV = self.V(hidden_states)
        x = torch.mul(self.Swish_1(xW),xV)
        x = self.W2(x)
        return x
